...
core_pa_global<-pa_optim(pu=pu,lockout = "built-up",lockin_col="exisiting_corePA", features =features, area_budget = gap[gap$lulc_name == "global_0.1_prot", ]$target_area/10^6)
...
pu <- pu %>%
  mutate(core_pa_lulc = case_when(
    exisiting_corePA == TRUE   ~ "existing core pa",
    optim_PA_single_lulc == 1 & n_pa == 0 ~ "new core PA",
    optim_PA_single_lulc == 1 & n_pa >0 ~ "proposed upgrade existing PA",
    exisiting_corePA == FALSE & is.na(optim_PA_single_lulc) & n_pa == 0 ~ "not protected",
    exisiting_corePA == FALSE & is.na(optim_PA_single_lulc) & n_pa > 0 ~ "other protected areas"
  ))
...
pu <- pu %>%
  mutate(core_pa_global = case_when(
    ...
    exisiting_corePA == FALSE & is.na(optim_PA_global)  & n_pa == 0 ~ "not protected",
    exisiting_corePA == FALSE & is.na(optim_PA_global)  & n_pa > 0 ~ "other protected areas"
  ))
